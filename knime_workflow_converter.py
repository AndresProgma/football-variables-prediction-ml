...
# ============================================================================
# 10B. ENTRENAR REGRESORES — predice goles de cada equipo
# ============================================================================
...
def main(filepath, test_size=0.2, random_state=42):
    ...
    # 11. Entrenar clasificadores (resultado) y regresores (goles)
...
# ============================================================================
def predecir_partido(equipo1, equipo2, results):
    """
    Predice el resultado de un partido futuro usando los perfiles
    históricos de ambos equipos (promedios de sus estadísticas).

    Args:
        equipo1:  Nombre del primer equipo (debe coincidir con el dataset)
        equipo2:  Nombre del segundo equipo
        results:  Diccionario devuelto por main()
    """
    df         = results['df']
    df_model   = results['df_model']
    models     = results['models']
    regressors = results.get('regressors', {})
    ...
    # Si el equipo no tiene historial como E1, usar sus stats de E2 renombradas (y viceversa)
    ...
    row = {'Es_Local_E1': 1}  # Equipo1 siempre es local
    ...
    if regressors:
        ...
        print(f"  {'-'*35}")
        for reg_name, reg_pair in regressors.items():
            g1 = max(0, round(reg_pair['E1'].predict(X_pred)[0]))
            g2 = max(0, round(reg_pair['E2'].predict(X_pred)[0]))
            print(f"  {reg_name:<16}  {equipo1} {g1} – {g2} {equipo2}")

    # --- Predicción de resultado ---
    print(f"\n  {'Modelo':<22}  Resultado   Probabilidades")
    print(f"  {'-'*60}")
    le_res = le_dict['Resultado_E1']
    for model_name, model in models.items():
        # SVM y KNN se guardan como (modelo, scaler)
        if isinstance(model, tuple):
            clf, scaler = model
            X_input = scaler.transform(X_pred)
        else:
            clf, X_input = model, X_pred

        pred_enc   = clf.predict(X_input)[0]
        pred_label = le_res.inverse_transform([pred_enc])[0]

        if hasattr(clf, 'predict_proba'):
            proba = clf.predict_proba(X_input)[0]
            proba_str = '  |  '.join(
                f"{cls}: {p:.0%}" for cls, p in zip(le_res.classes_, proba)
            )
            print(f"  {model_name:<22} → {pred_label:<6}  ({proba_str})")
        else:
            print(f"  {model_name:<22} → {pred_label}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        predecir_partido("Union SG",  "Inter",       results)
        predecir_partido("Newcastle",    "Benfica", results)
        predecir_partido("Barcelona",    "Paris",          results)
